...
export default function Home() {
  ...
  const BENEFITS = [
    ...
    },
    {
      ...
      desc: 'ยอดเงิน ชื่อผู้โอน หมวดหมู่ค่าใช้จ่าย — บันทึกอัตโนมัติทันทีทุกครั้งที่สแกนสลิป ดูสดผ่าน Dashboard ได้เลย',
    ...
    },
    {
      ...
      desc: 'รูปสลิปทุกใบสำรองเข้า Google Drive ส่วนตัวของร้านคุณด้วย แยกโฟลเดอร์ตามเดือน — เก็บรักษาตามมาตรฐาน PDPA ไม่ขาย/เปิดเผยข้อมูลให้บุคคลที่สาม',
      items: ['สำรองรูปสลิปเข้า Google Drive ของร้าน', 'แยกโฟลเดอร์ปี/เดือน', 'Export Excel รายเดือนได้'],
    },
    {
  ...
  const COMPARISON_ROWS = [
    ...
    { label: 'ข้อมูลเป็นของใคร', old: 'ล็อกไว้ ย้ายออก/เลิกใช้ยาก', us: 'Export Excel/ลบได้ตลอด ไม่ล็อกคุณไว้' },
  ];
  ...
  const TIERS = [
    {
      ...
      features: ['สแกนสลิปอัตโนมัติ + POS เต็มรูปแบบ', 'สต็อกหมุนเวียน/แอปพนักงานส่ง/เงินเชื่อ', 'สำรองรูปสลิปเข้า Google Drive ของร้าน'],
    ...
    },
    {
      ...
      features: ['10 สาขา · 3 Admin', 'รายงานภาษี VAT (ภ.พ.30)', 'ระบบเงินเดือน + ประกันสังคม', 'Top Sender Analysis'],
      cta: 'เลือกแพ็กเกจนี้', popular: false,
    },
    {
  ...
  const FAQS = [
    { q: 'ข้อมูลการเงินของร้านปลอดภัยไหม?', a: 'ปลอดภัยครับ ข้อมูลธุรกรรม (ยอดเงิน, ชื่อผู้โอน) เก็บไว้ในระบบที่มีมาตรฐานความปลอดภัยระดับองค์กร เราทำหน้าที่เป็นแค่ผู้ให้บริการเก็บรักษาแทนร้านค้า ไม่ใช่เจ้าของข้อมูล ไม่ขาย/เปิดเผยให้บุคคลที่สามเด็ดขาด ส่วนรูปสลิปสำรองไว้ใน Google Drive ของร้านคุณเองด้วยเสมอ เป็นไปตาม PDPA' },
    ...
    { q: 'ถ้ายกเลิก ข้อมูลหายไปไหม?', a: 'ยกเลิกแพ็กเกจรายเดือนอย่างเดียวไม่มีอะไรหายเลยครับ ข้อมูลยังอยู่ครบ กลับมาใช้ต่อได้ตลอด — ถ้าลบบัญชีร้านออกจากระบบ เราจะเก็บข้อมูลไว้ให้อีก 6 เดือนเผื่อกลับมาใช้ใหม่ (สมัครซ้ำแล้วเลือก "ใช้ข้อมูลเดิม" ได้เลย) ก่อนลบถาวรอัตโนมัติ ส่วนรูปสลิปยังอยู่ใน Google Drive ของร้านคุณเองเสมอไม่ว่ากรณีใด' },
  ];
                ...
                <div className="relative">
                  ...
                  <div className="absolute -bottom-3 -left-3 bg-white rounded-2xl shadow-xl px-3 py-2 flex items-center gap-2 border border-slate-100">
                    ...
                    <div>
                      ...
                      <p className="text-emerald-500 text-[9px]">เข้ารหัสปลอดภัย</p>
                    </div>
                  </div>
          ...
          <div className="max-w-4xl mx-auto px-5 text-center">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-8 relative">
              ...
              {[
                ...
                { step:'03', icon:'📊', title:'บัญชีอัปเดตอัตโนมัติ', desc:'ข้อมูลบันทึกเข้าระบบทันที เจ้าของรับแจ้งเตือน ดู Dashboard ได้เลย' },
              ...
              {TIERS.map((t, i) => {
                ...
                return (
                  <div key={i} className={`bg-white rounded-2xl border-2 p-5 relative flex flex-col ${c.border}`}>
                    ...
                    <Link href={t.name === 'Enterprise' ? 'https://lin.ee/o0ytFIG' : '/register'}
