...
อัปเดตล่าสุด: 2026-07-21 (ทำ 3 ใน 5 งาน "ง่ายก่อน" เสร็จครบแล้ว: ตารางเปรียบเทียบแพ็กเกจ, เว็บสั่งซื้อสาธารณะสำหรับลูกค้า (`pages/order.js`), Excel report engine (3 แม่แบบใหม่ + custom scaffold), และล่าสุด **สิทธิ์พนักงานละเอียด** (4 สิทธิ์แยกต่อคน: ดูยอดขาย/กำไรขาดทุน/จัดการสต็อก/export VAT — enforce จริงฝั่ง server ผ่าน `staffId` ที่ไม่บังคับ ไม่กระทบเจ้าของร้านเลย) — ดูข้อ 35 ในหัวข้อ "เหตุการณ์และบั๊กที่แก้แล้ว" — **Deploy production แล้ว (2026-07-21, revision จริง `smileslip-dashboard-00269-lpk` — ข้อความ deploy โชว์ revision เดิม `00268-slt` ผิดอีกตามเคย เช็คด้วย `gcloud run revisions list` แล้ว pin traffic เอง)** — verified บน production จริง: `/pricing`, `/order`, `/api/pos/public-shop-info`, `/api/pos/staff` (คอลัมน์สิทธิ์ใหม่), `/api/pos/reports?type=pl` (ทางเจ้าของร้านไม่ส่ง staffId ยังทำงานปกติ) — หลังจากนั้นทำ **White-Label MVP เพิ่ม** (ดูข้อ 36) — ตัดสโคปจาก "ผูก LINE OA ลูกค้าเอง" (ใหญ่/ยังไม่ทำ) เหลือแค่ "ลบชื่อ Smile Slip Pro ออกจากบิล/ใบเสร็จ/ใบกำกับภาษี/ชื่อไฟล์ export ให้ Enterprise" — **Deploy production แล้ว (2026-07-21, revision จริง `smileslip-dashboard-00270-p5n` — ข้อความ deploy โชว์ revision เดิม `00269-lpk` ผิดอีกตามเคย เช็คด้วย `gcloud run revisions list` แล้ว pin traffic เอง)** — verified บน production จริง: `/api/pos/public-shop-info` คืน `isWhiteLabel:true` ถูกต้องสำหรับร้าน D Gas (enterprise), `/api/export/excel` ชื่อไฟล์ไม่มี prefix `SmileSlip_` แล้วจริง — เหลือ: โอนย้ายสต็อกข้ามสาขา (deferred รอคุยโมเดลสต็อกแยกสาขา), White-Label แบบ LINE OA เต็มรูปแบบ (ยังไม่เริ่ม รอคุยรายละเอียด), **QR สั่งอาหารที่โต๊ะ — วางแผนเฟสแล้ว ตัดสินใจไม่เริ่มตอนนี้ (ยังไม่มีลูกค้าร้านอาหารจริง) ดูข้อ 37** — ล่าสุดทำ **สินค้า/ลิงก์สั่งซื้อแยกต่อสาขา + แก้บั๊กลิงก์แผนที่หายในหน้ายืนยันจัดส่ง** (ดูข้อ 38) — หลังจากนั้นทำ **QA Stress Test เต็มรูปแบบ + แก้บั๊กที่เจอ 5 ข้อ** (ดูข้อ 39 — สต็อคเสกได้จาก qty ติดลบ, ราคาปลอมไม่มีใครเช็ค, ยกเลิกบิลไม่ได้เลย ฯลฯ) — **Deploy production แล้ว (2026-07-22, revision จริง `smileslip-dashboard-00271-g49`)** — ผู้ใช้ขอเช็คซ้ำแบบละเอียดอีกรอบหลัง deploy พบเพิ่มอีก 3 จุด (ดูข้อ 40 — products.js/contacts.js ไม่กันค่าติดลบตั้งแต่สร้าง, tax-invoice.js ไม่กันจำนวน/ราคาติดลบ, จำกัด qty เว็บสั่งซื้อสาธารณะ) แก้ครบ + **Deploy production ครบทั้ง 3 รอบแล้ว (revision จริงล่าสุด `smileslip-dashboard-00273-45d`) — verified บน production จริงว่า tax-invoice.js ปฏิเสธ qty ติดลบถูกต้อง** — commit ครบ 4 รอบ (`53e7753`, `5990612`, `dd49db9`, `55e3f82`))
...
41. **ระบบเงินสด/ปิดกะ (เปิดกะ-ปิดกะ) + คำถามเปิดลิ้นชักเงินสดอัตโนมัติ — ผู้ใช้แจ้งว่าลืมพูดถึงฟีเจอร์นี้ กำลังวางแผน/คุยรายละเอียดอยู่ ยังไม่เริ่มเขียนโค้ด:**
    - **สิ่งที่ผู้ใช้ต้องการ (ตามที่อธิบาย):** เปิดกะ → คีย์ยอดเงินสดตั้งต้นในลิ้นชัก → ระหว่างกะระบบต้องรู้เงินสดเข้า (ขายเงินสด) และเงินสดออก (เงินทอน + รายจ่ายที่จ่ายด้วยเงินสด) อัตโนมัติจากข้อมูลที่มีอยู่แล้ว (sales.js/expenses.js) → ปิดกะ → นับเงินสดจริงเทียบกับยอดที่ควรมีตามระบบ (เช็คส่วนต่าง/เงินขาด-เกิน) → บันทึกยอดที่เก็บออกไป (ฝากธนาคาร/เข้าตู้เซฟ) กับยอดที่ยกไปเป็นทุนตั้งต้นกะถัดไปแยกกัน
    - **คำถามเรื่องเปิดลิ้นชักอัตโนมัติ:** ผู้ใช้ไม่แน่ใจว่าใช้ระบบไหน — ต้องอธิบาย/คุยรายละเอียดเรื่องข้อจำกัดของเบราว์เซอร์กับฮาร์ดแวร์ลิ้นชักเงินสด (ปกติเปิดผ่านสัญญาณ kick-drawer จากเครื่องพิมพ์ใบเสร็จที่เสียบสาย ไม่ใช่ผ่าน `window.print()` ที่ระบบใช้อยู่ตอนนี้) — คล้ายกับข้อจำกัดเดียวกับที่เคยตัดสินใจไม่ทำ Web Bluetooth ESC/POS สำหรับพิมพ์ใบเสร็จโดยตรงในข้อ 22 (ต้องรู้ยี่ห้อ/รุ่นเครื่องจริงถึงจะทำได้ ไม่มีมาตรฐานเดียวที่ใช้ได้ทุกรุ่น)
    - **ยังไม่ได้ตัดสินใจ/ออกแบบละเอียด** — รอคุยกับผู้ใช้ต่อ (เช่น ปิดกะเป็นต่อสาขาหรือต่อคนขาย, ทำงานร่วมกับระบบหลายบิลที่มีอยู่แล้วยังไง)
