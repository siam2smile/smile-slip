...
อัปเดตล่าสุด: 2026-07-19 (แก้บั๊ก asText() วันที่เพี้ยนหลุดใน API pos อีก 7 ไฟล์ — ดูข้อ 27 ในหัวข้อ "เหตุการณ์และบั๊กที่แก้แล้ว")
...
26. **แก้บั๊ก Peak Time Heatmap (เวลาเพี้ยน 7 ชม.) + ระบบ PIN พนักงานรายบุคคล + ล้างสถิติสลิปเก่า — งานชุดใหญ่ที่ผู้ใช้ขอสรุปก่อนเริ่มทำอีกรอบ (VAT/รายจ่ายยังไม่เสร็จ อยู่ระหว่างทำต่อ):**
    ...
    - **กำลังทำต่อ:** ระบบ VAT ครบวงจร (คำนวณ VAT ตอนขายหน้าร้าน, toggle "ร้านจด VAT" ในตั้งค่า, รูปแบบใบกำกับภาษีทางการแบบเดียวกับที่แอดมิน Smile Slip Pro ออกให้ตอนซื้อแพ็กเกจ, ตัวเลือกราคารวม/ไม่รวม VAT ตอนรับสินค้า, รายงาน VAT ขาย+ซื้อแยกสาขา) + หน้า "รายจ่าย" ใหม่ในระบบ POS (ไม่เกี่ยวกับสต็อค มี VAT/ไม่มี VAT + แนบรูปบิล) + เชื่อมกับหน้ากราฟวิเคราะห์
27. **ตามรอยแก้บั๊ก "วันที่เพี้ยนเป็นเลขซีเรียล/เพี้ยน 543 ปี" (asText() helper) ให้ครบทุกไฟล์ใน `api/pos/` — ระหว่างสร้าง `expenses.js` (ฟีเจอร์ "รายจ่าย" ใหม่ ยังทำอยู่คนละไฟล์/คนละ session ไม่ commit ที่นี่) เจอ pattern เดิมที่เคยแก้ไปแล้วในข้อ 14/19 (เขียน `new Date().toLocaleString('th-TH', {timeZone:'Asia/Bangkok'})` ตรงๆ ลง Google Sheets ด้วย `USER_ENTERED` ทำให้ Sheets auto-parse เป็นเลขซีเรียลดิบ หรือถ้ามีปี พ.ศ. ปนจะเพี้ยนไปข้างหน้า ~543 ปี) หลุดอยู่ใน API route อื่นอีกจำนวนมากที่ไม่เคย wrap ด้วย `asText()` เลย — grep เจอ 10 ไฟล์ (`collections.js`, `loans.js`, `delivery.js`, `contacts.js`, `products.js`, `staff-requests.js`, `tax-invoice.js`, `staff.js`, `sales.js`, `receives.js`) ตรวจแล้วพบว่า `contacts.js`/`staff.js`/`staff-requests.js` จริงๆ ถูก wrap ไว้แล้วครบ (แก้ไปแล้วในรอบก่อนหน้าโดยไม่ได้ log ไว้ตรงนี้) เหลือ 7 ไฟล์ที่มีจุดหลุดจริง — แก้โดยเพิ่ม helper `asText()` (แพทเทิร์นเดียวกับที่ project นี้ใช้อยู่แล้ว — duplicate ต่อไฟล์ ไม่ centralize) ให้ `loans.js`, `products.js`, `tax-invoice.js`, `sales.js`, `receives.js` ที่ยังไม่มี และ wrap ทุกจุดเขียน timestamp (`now`/`existing[9]`/`custExisting[19]`/`prodExisting[9]` ฯลฯ) ด้วย `asText()` ให้ครบทั้ง 7 ไฟล์ (รวม `collections.js` และ `delivery.js` ที่มี helper อยู่แล้วแต่ลืม wrap หลายจุด)
    - **ทดสอบยิงจริงครบทุก endpoint ที่แก้ผ่าน dev server + curl ตรง (ไม่ผ่าน browser UI) กับร้านทดสอบจริง "D Gas"** (`shop_id=f8009a36-9331-40c8-9664-ab255ebbd35c`): products (POST เพิ่ม + PATCH generic), contacts (POST ยืนยันซ้ำว่าถูกอยู่แล้ว), loans (POST + PATCH คืนของ), sales (POST ขายเชื่อ + PATCH รับชำระ + ผลกระทบสต็อคสินค้า), receives (POST รับสินค้า), tax-invoice (POST ออกใบกำกับภาษี), collections (POST + PATCH ผลเก็บเงิน/เก็บของ ทั้งกับลูกค้าและกับสต็อคสินค้าหมุนเวียน), delivery (POST ค้างจ่าย + PATCH confirm_delivery คืนถัง + PATCH credit_settled) — ทุกจุดคืนวันที่แบบอ่านได้ (เช่น `"19/7/2569 14:35:37"`) ไม่ใช่เลขซีเรียลอีกต่อไป — ลบข้อมูลทดสอบที่ลบได้ทั้งหมด (contact/product/collection/delivery order มี DELETE endpoint) และคืนค่าตัวเลขสต็อคสินค้าจริงของร้าน (`PN0LEOJ` "น้ำแก๊ส 7 กก." — `empty_waiting` ที่ขยับระหว่างทดสอบ) กลับที่เดิม — เหลือ 3 รายการที่ลบไม่ได้เพราะไฟล์ `loans.js`/`sales.js`/`tax-invoice.js` ไม่มี DELETE endpoint (บิลขาย 2 ใบ + ใบยืม 1 ใบ + ใบกำกับภาษี `INV-2569-00001` 1 ใบ) แต่ทุกรายการ label ชัดเจนว่าเป็นข้อมูลทดสอบ (`TEST-asText-contact`/`TEST-asText-products`/cashier `"Claude test"`)
    - **พบระหว่างทาง (ไม่ได้แก้ ไม่ใช่ scope ของงานนี้):** `writeToMainSheets()` ใน `sales.js` เขียน `thaiDate`/`thaiTime` (จาก `toLocaleDateString`/`toLocaleTimeString`) ลง Main Sheets โดยไม่ผ่าน `asText()` เช่นกัน — คนละ pattern จากที่ grep เจอรอบนี้ (ใช้ `toLocaleDateString`/`toLocaleTimeString` ไม่ใช่ `toLocaleString`) ยังไม่ได้ตรวจว่าเสี่ยงบั๊กเดียวกันหรือไม่ เป็น known gap ถ้าเจอปัญหาวันที่เพี้ยนในบัญชีหลัก (ไม่ใช่ POS Sheets) ให้ตรวจจุดนี้ก่อน; และพบว่า `loans.js` เขียนเบอร์โทรผู้ยืม (`contact_phone`) ดิบไม่ผ่าน `asText()` เช่นกัน (หาย 0 หน้าแบบเดียวกับบั๊กข้อ 14/19) — ไม่ได้แก้เพราะนอก scope ที่ผู้ใช้ขอ (ขอบเขตงานนี้คือ timestamp เท่านั้น)
    - **พบไฟล์ `expenses.js` (ฟีเจอร์ "รายจ่าย" ที่ผู้ใช้บอกว่าทำเสร็จแล้ว) อยู่ใน working directory หลัก (`D:/Project/smile-slip-pro`) ไม่ใช่ worktree นี้ และยังไม่เคย commit เข้า git เลย (`git status` แสดง `??`)** — ตรงกับ pattern ความเสี่ยงเดียวกับที่เคยเกิดกับ `pos.js`/`pos-staff.js`/`pos-config.js` มาก่อน (ไฟล์ที่มีค่ามากแต่ไม่มี backup ใน git) — ไม่ได้แตะ/commit ให้เพราะอยู่นอก worktree ของ session นี้ (อาจมี session อื่นทำงานอยู่พร้อมกัน) **ต้องเตือนผู้ใช้ให้ไป commit ไฟล์นี้ในเครื่องหลักเองโดยเร็ว**
