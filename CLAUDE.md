...
อัปเดตล่าสุด: 2026-07-23 (ต่อจาก QA Stress Test 3 รอบ (ข้อ 39-40) — ผู้ใช้ขอ **ระบบเปิดกะ/ปิดกะเงินสด** ต่อพนักงานผ่าน PIN (ดูข้อ 41) เก็บยอดเงินสดตั้งต้น/ขายเงินสด/รายจ่ายเงินสดอัตโนมัติ คำนวณเงินสดที่ควรมีใหม่จากข้อมูลจริงเสมอตอนปิดกะ ยอดไม่ตรงไม่บล็อคแต่บังคับหมายเหตุ+แจ้งเจ้าของร้านทาง LINE — ระหว่างทดสอบเจอบั๊กจริง 2 จุด: (1) `ensureTabExists()` เสี่ยงพังข้อมูลจริงเงียบๆ ถ้าโดน Sheets API quota exceeded (เข้าใจผิดว่า tab ไม่มีอยู่จริงแล้วเขียน header ซ้ำทับข้อมูลจริง) — แก้เป็น fail loud แทน กระทบทุก POS route ในระบบไม่ใช่แค่ฟีเจอร์นี้ (2) เขียนเวลาเปิด/ปิดกะไม่ผ่าน `asText()` เสี่ยงเพี้ยน ~543 ปีแบบเดิม — **Deploy production แล้ว (2026-07-23, revision จริง `smileslip-dashboard-00274-g98` — ข้อความ deploy โชว์ revision เดิม `00273-45d` ผิดอีกตามเคย เช็คด้วย `gcloud run revisions list` แล้ว pin traffic เอง)** — verified บน production จริง: `/api/pos/verify-pin` (purpose=cash_shift) และ `/api/pos/cash-shifts` ทำงานถูกต้อง — เหลือ: เปิดลิ้นชักเงินสดอัตโนมัติ (ต้องมีเครื่องพิมพ์จริงก่อนถึงจะทำได้ ตัดสินใจไม่ทำตอนนี้), โอนย้ายสต็อกข้ามสาขา (deferred), White-Label แบบ LINE OA เต็มรูปแบบ (ยังไม่เริ่ม), QR สั่งอาหารที่โต๊ะ (วางแผนแล้ว ยังไม่เริ่ม))
