...
อัปเดตล่าสุด: 2026-07-20 (ทำฟีดแบ็ก 9/11 ข้อจากผู้ใช้ — bills sidebar, ใบกำกับภาษี+ผู้ติดต่อ, ตัวกรองเก็บเงิน/ของ, หมวดหมู่รายจ่าย+กราฟ, ส่วนลด+พิมพ์สลิปจัดส่ง, สรุปยอดบัญชี, แก้บั๊กอัลบั้มรูป LINE, เริ่มระบบ multi-admin (`company_admins` — รอรัน SQL) — ดูข้อ 32 ในหัวข้อ "เหตุการณ์และบั๊กที่แก้แล้ว", งานใหญ่ 2 ข้อเหลือ: อัปเกรดหน้าบัญชี→ภาษี, ทดสอบ multi-admin end-to-end หลังรัน SQL — **ข้อ 32 ยังไม่ได้ deploy**)
...
32. **ฟีดแบ็ก 11 ข้อจากไฟล์ผู้ใช้ส่งมา (`เพิ่มเติม 20072569-1.txt`) — ทำครบ 9/11 ข้อในรอบนี้ (2 ข้อสุดท้ายเป็นงานใหญ่แยกทำต่อ):**
    ...
    - **ข้อ 8 ส่วนที่เหลือ (ยังไม่ทำ — งานใหญ่แยกเป็นงานต่อไป):** อัปเกรดหน้า "บัญชี → ภาษี" (ผู้ใช้ยืนยันว่าหมายถึงหน้านี้ ไม่ใช่ POS→รายงาน→VAT ที่ทำไว้แล้วในข้อ 27) ให้เป็นรายงานภาษีซื้อ-ขายแยกตามสาขา/รวมทุกสาขาได้ + export ให้ใช้ยื่นบัญชีกับสำนักงานบัญชีภายนอกได้จริงโดยไม่ต้องพึ่งโปรแกรมอื่น — ยังไม่ได้เริ่ม
    - **ข้อ 9 (multi-admin) — ทำเสร็จแล้ว:** แทนที่ `ADMIN_PASSWORD` เดี่ยวด้วยระบบบัญชีแอดมินจริง (ผู้ใช้เลือก "ทำ multi-admin เต็มรูปแบบตั้งแต่ตอนนี้เลย" + "เก็บ ADMIN_PASSWORD ไว้เป็น fallback ฉุกเฉิน")
      ...
      - **ทดสอบแล้ว (ก่อนรัน SQL):** ยิง `curl` ตรงยืนยันว่า legacy login ยังทำงานปกติ 100% แม้ตาราง `company_admins` ยังไม่มีอยู่จริง (`needsBootstrap:false` ถูกต้อง, ไม่ crash), email-login คืน error ชัดเจนว่ายังไม่ได้ตั้งค่า, `GET /api/admin/admins` ไม่มี token ถูกปฏิเสธถูกต้อง — **ยังไม่ได้ทดสอบ end-to-end เต็มรูปแบบ (bootstrap จริง → สร้างแอดมิน → login → เปลี่ยนรหัสผ่าน → ระงับ/เปิดใช้งาน) เพราะต้องรอรัน SQL ก่อน**
    - **ยังไม่ได้ deploy ข้อ 32 นี้เลย** (ต้อง deploy dashboard หลังรัน SQL `company_admins` เพื่อทดสอบ end-to-end ให้ครบก่อน)
