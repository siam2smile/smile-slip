...
21. **ตรวจสอบ LINE push แจ้งเตือนพนักงานส่ง — ยิงทดสอบตรง 3 ครั้ง (ข้อความธรรมดา + Flex Message จริง) LINE ตอบสำเร็จทุกครั้งพร้อม message ID จริง แต่ผู้ใช้ยืนยันว่าไม่มีข้อความขึ้นที่ปลายทางเลย** — ตรวจ `bot/info` ด้วย token เดียวกันแล้วยืนยันว่าเป็น OA `@574unjqj` "Smile Slip" ตัวเดียวกับบอทจริง ไม่ใช่คนละ channel — ยังหาสาเหตุไม่เจอ ณ ตอนนี้ สงสัยว่า LINE user ID ของ "Testing" (ผู้ที่พิมพ์ `#สมัครพนักงานขนส่ง` ตอน 11:37 น. วันก่อน) อาจไม่ใช่เครื่อง/บัญชีที่ผู้ใช้เช็คอยู่จริง — **รอผู้ใช้ยืนยัน** ว่าเป็นเครื่องเดียวกับที่สมัครไว้ไหม และลองเปิดแชท "Smile Slip" ตรงๆดูประวัติ (ไม่ใช่แค่ดู notification popup)
